...
_INSTALL_SUCCESS_INFO="Terminal needs to be restarted to run it!"
...
function _i_prompt() {
  ...
  # 'PS1' is generated by _prompt file,so the file is not needed.
  local git_pr='/etc/profile.d/git-prompt.sh'
  [ -f "$git_pr" ] && _backup_file "$git_pr"

  # 'git_cpl' only exist in windows, it's used for git command completion
...
function _echo_run_source() {
  ...
  echo -n "Run: "
  _echo "reverse" "echo '$1'>>${profile_path}"
}
...
function _alone_install() {
  ...
  file_name="${file_name^}"
  _echo "success" "${file_name} of wgc succeed install!"
  _echo_run_source "source '${_INSTALL_PATH}/${file_name}'"
  _echo "success" "$_INSTALL_SUCCESS_INFO"
}
...
function _wgc_install() {
  ...
  _echo_run_source "source ${start_path}main"
  _echo "success" "$_INSTALL_SUCCESS_INFO"
}
